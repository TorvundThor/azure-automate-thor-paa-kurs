...
function skrivUt {
    param (
        ...
        Write-Output "trukket: " $(listKort -stokk $trukket)
        Write-Output "summen av kortene er:  $(summerKort -kortstokk $trukket)"

        Write-Output "meg: " $(listKort -stokk $megListe)
        Write-Output "Magnus: " $(listKort -stokk $magnusListe)

        Write-Output "summen av kortene er:  $(summerKort -kortstokk $megListe)"
        Write-Output "summen av kortene er:  $(summerKort -kortstokk $magnusListe)"
...
$i = 1
$magnus = $kortstokk[0..$i]
$i = $i + 1
$kortstokk = $kortstokk[$i..$kortstokk.Count]
...
while ($(summerkort -kortstokk $magnus) -lt 21)
    {
        $i = 0
        $magnus = $magnus + $kortstokk[0..$i]
        $i = $i + 1
        $kortstokk = $kortstokk[$i..$kortstokk.Count]
        if ($(summerkort -kortstokk $magnus) -ge 17) {break}
    }
}
